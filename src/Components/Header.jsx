...
const Header = () => {
    ...
    return (
        <nav className={`fixed w-full z-[100] transition-all duration-500 ${scrolled
            ...
            {/* MOBILE MENU */}
            {menu && (
                <div className="md:hidden absolute top-full left-0 w-full bg-white border-b shadow-xl py-4 px-6 flex flex-col gap-2 animate-in fade-in slide-in-from-top-2">
                    {navLink.map((val) => (
                    ...
                    {user ? (
                        <>
                            ...
                            <Link to="/profile" onClick={() => setMenu(false)} className="py-3 font-bold text-slate-700 border-b border-slate-50">My Profile</Link>
                            <button onClick={handleLogOut} className="py-3 font-bold text-red-600 text-left uppercase text-xs tracking-widest mt-2 flex items-center gap-2">
                        ...
                        <Link 
                            ...
                            className="my-2 flex items-center justify-center gap-2 w-full py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-colors shadow-sm"
                        >
